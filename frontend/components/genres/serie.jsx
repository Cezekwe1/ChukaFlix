...
import DetailIndexItem from './detail';
import EpisodeIndexItem from './episode';
...
class Serie extends React.Component {
  constructor (props) {
    ...
    const currentUserReview = this.props.serie.reviews.find((review) => {
      if (review.profile_id === this.props.profile.id){
        return review;
      }
    });
  ...
  onStarClick(nextValue, prevValue, name) {
    let currentUserId = this.props.profile.id;
    const currentUserReview = this.props.serie.reviews.find((review) => {
      if (review.profile_id === this.props.profile.id) {
        return review;
      }
  ...
  handleMyListClick(serie,profile) {
  ...
  renderMyListToggle() {
    ...
    return (
      <button
        ...
        onClick={ () => this.handleMyListClick(this.props.serie, this.props.profile)}>{myListButtonText}</button>
    );
  }
  ...
  render () {
    ...
    const episodeIndexItems = this.props.serie.episodes.map((episode, id) => {
      if (episode.video_url === this.state.currentVideoId) {
        return (<div key={episode.id} className="episode-group group">
          ...
          <p className="episode-synopsis">{episode.description}</p>
        </div>
        );
      } else {
      return (
    ...
    const genre = this.props.serie.genre.title
        ...
        >
          ...
          <div className="top-modal group">
            ...
            <section className="other-details">
              ...
              <ul className="serie-genres">Genre: {genre}</ul>
            </section>
          </div>
